function [time_occurs, P_peak_values] = find_P_peaks(ecg, Fs, S_locations, Q_locations, tmin)
...
%occur
%P_peak_values is a vector which contains the values of the different T
%peaks

P_peak_values = zeros(1, length(S_locations)-1);
...
for i=1:length(S_locations)-1
    ...
    ecg_tmp2 = ecg_tmp(1+floor(length(ecg_tmp)*0.7):length(ecg_tmp));
    [P_peak_values(i), time_occurs(i)] = max(ecg_tmp2);
    time_occurs(i) = time_occurs(i) + S_locations(i) + floor(length(ecg_tmp)*0.7) - 1;
end
